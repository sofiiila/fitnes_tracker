...
class TestRunning(unittest.TestCase):
    ...
    def test_get_spent_calories(self):
        running = Running(15000, 1, 70)
        expected_calories = 246
    ...
    def test_show_training_info(self):
        ...
        expected_message = "Тип тренировки: Running; Длительность: 1.000 ч.; Дистанция: 9.750 км; Ср. скорость: 9.750 км/ч; Потрачено ккал: 225.000."
        self.assertEqual(running.show_training_info().get_message(), expected_message)

class TestSportsWalking(unittest.TestCase):
    """Тестирование класса SportsWalking"""
    def test_get_spent_calories(self):
        sports_walking = SportsWalking(9000, 1, 75, 180)
        expected_calories = 176
        self.assertAlmostEqual(sports_walking.get_spent_calories(), expected_calories)

    def test_show_training_info(self):
        sports_walking = SportsWalking(9000, 1, 75, 180)
        expected_message = "Тип тренировки: SportsWalking; Длительность: 1.000 ч.; Дистанция: 5.850 км; Ср. скорость: 5.850 км/ч; Потрачено ккал: 176.000."
        self.assertEqual(sports_walking.show_training_info().get_message(), expected_message)
...
class TestSwimming(unittest.TestCase):
    ...
    def test_show_training_info(self):
        ...
        self.assertEqual(swimming.show_training_info().get_message(), expected_message)
